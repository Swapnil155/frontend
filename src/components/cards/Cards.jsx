...
const Cards = ({data, pagination}) => {
  ...
  const MainPathName = MainPath[0]
  // console.log(MainPathName)
  return (
      
    <div className="row">
    {data.slice(pagination.start,pagination.end).map((elem, i) => {
      const { Tour_Image, Title, Tour_Price, Tour_Location, _id, Tour_author } = elem;
      return (
        <div className="col-lg-4 col-sm-6 mb-4" key={i}>
          <div className="card h-100 border-0 shadow ">
            <div className="card-img-top  overflow-hidden gradient-overlay">
              <Link to={`${MainPathName}/State/Tour-Details/${Title.slug}&${_id}`}>
                <img src={Tour_Image.coverImage} className="img-fluid" alt="..." />
              </Link>
              {/* Hosted by */}
            ...
            <div className="card-body">
              ...
              <p className="card-text text-muted">
                <span className="h4 fw-bold text-primary"><BiRupee  className='fw-700 fs-3 mb-1' />{Tour_Price.price}</span>
                per person
              </p>
            </div>
          </div>
